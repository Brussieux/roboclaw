...
class Roboclaw:
    ...
    def _sendcommand(self, address, command):
        ...
        # send bytes in py3
        self._port.write(int(address).to_bytes(1, 'big'))
        self.crc_update(command)
        self._port.write(int(command).to_bytes(1, 'big'))
        return
    ...
    def _writebyte(self, val):
        ...
        self._port.write(int(val & 0xFF).to_bytes(1, 'big'))
    ...
    def SendRandomData(self, cnt):
        for i in range(0, cnt):
            ...
            self._port.write(int(byte).to_bytes(1, 'big'))
        return
    ...
    def SetM1VelocityPID(self, address, p, i, d, qpps):
        return self._write4444(address, self.Cmd.SETM1PID, int(d * 65536), int(p * 65536), int(i * 65536), int(qpps))

    def SetM2VelocityPID(self, address, p, i, d, qpps):
        return self._write4444(address, self.Cmd.SETM2PID, int(d * 65536), int(p * 65536), int(i * 65536), int(qpps))
